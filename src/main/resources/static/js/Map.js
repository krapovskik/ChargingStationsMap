...
function filterStations(map){
    ...
    if(filter.value.length === 0)
        ourRequest.open('GET','http://127.0.0.1:8080/api/stations')
    else
        ourRequest.open('GET','http://127.0.0.1:8080/api/stations?filter='+filter.value)
...
function getCity(lat,lng) {
    var ourRequest = new XMLHttpRequest();
    ourRequest.open('GET', 'http://api.geonames.org/findNearbyPlaceNameJSON?lat='+ lat +'&lng='+ lng +'&username=demo123')
    ourRequest.onload = function () {
        var ourData = JSON.parse(ourRequest.responseText);
